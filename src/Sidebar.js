...
const NewSidebar = ({ isOpen, onClose }) => {
  ...
  const handleLinkClick = (event, isAnchorLink) => {
    if (isAnchorLink) {
    ...
    <div className={`new-open-sidebar ${isOpen ? 'new-show-sidebar' : ''}`}>
      ...
      <div className='new-sidebar'>
        <ul className='new-sidebar-lists'>
          <li onClick={() => { handleLinkClick(true); handleDropdownToggle(0); }}>
            <a href='#about'><h4>Who I Am</h4></a>
          </li>
          <li onClick={(e) => { handleLinkClick(e, false); handleDropdownToggle(1); }}>
          ...
          <li onClick={(e) => { handleLinkClick(e, false); handleDropdownToggle(2); }}>
